...
export class DrawerResizeController implements ReactiveController {
    ...
    public increment = 5; // px

    constructor(private host: DrawerResizeControllerHost) {
        /* no op */
    }
    ...
    handleMove(ev: MouseEvent | TouchEvent, controlPosition: number) {
        ...
        this.panel.style.setProperty("--pf-v5-c-drawer__panel--md--FlexBasis", `${newSize}px`);
    }
    ...
    handleKeyDown = (ev: KeyboardEvent) => {
        ...
        const delta = match([key, this.position])
            .with(["ArrowRight", oneOf("end", "right")], () => -1 * this.increment)
            .with(["ArrowLeft", oneOf("end", "right")], () => this.increment)
            .with(["ArrowRight", oneOf("start", "left")], () => this.increment)
            .with(["ArrowLeft", oneOf("start", "left")], () => -1 * this.increment)
            .with(["ArrowUp", "bottom"], () => this.increment)
            .with(["ArrowDown", "bottom"], () => -1 * this.increment)
            .otherwise(() => 0);
