...
To get authentik to automatically restart whenever code changes are made, install [watchexec](https://github.com/watchexec/watchexec), and run:

```shell
make run-watch
```

### Running only the server or the worker

To run only the server or the worker, use the following commands:
...
And their automatically reloading equivalents are:

```shell
make run-server-watch
```
