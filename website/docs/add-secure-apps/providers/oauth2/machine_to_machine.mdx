...
If you want to carry claims from the Kubernetes token into the token authentik issues, use an **OAuth Source Property Mapping** to copy values from the incoming JWT onto the generated service account, and then use an OAuth2 **Scope Mapping** to emit those values as claims for the relying party.
