...
class PolicyEvaluator(BaseEvaluator):
    ...
    def _make_has_access_to_application(self, request: PolicyRequest):
        ...
        def has_access_to_application() -> bool:
            """Check if the current agent user's owner has access to the current application.
            Returns False when called outside an application-access policy context, or when
            the current user is not an agent user."""
            ...
            if not getattr(user, "attributes", None):
                return False
            if not user.attributes.get(USER_ATTRIBUTE_IS_AGENT):
                return False
