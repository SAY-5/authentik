...
/// struct for typed errors of method [`managed_blueprints_import_create`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ManagedBlueprintsImportCreateError {
    Status400(models::BlueprintImportResult),
    Status403(models::GenericError),
    UnknownValue(serde_json::Value),
}
...
pub async fn managed_blueprints_apply_create(configuration: &configuration::Configuration, instance_uuid: &str) -> Result<models::BlueprintInstance, Error<ManagedBlueprintsApplyCreateError>> {
    ...
    let uri_str = format!("{}/managed/blueprints/{instance_uuid}/apply/", configuration.base_path, instance_uuid=crate::apis::urlencode(p_path_instance_uuid));
...
pub async fn managed_blueprints_destroy(configuration: &configuration::Configuration, instance_uuid: &str) -> Result<(), Error<ManagedBlueprintsDestroyError>> {
    ...
    let uri_str = format!("{}/managed/blueprints/{instance_uuid}/", configuration.base_path, instance_uuid=crate::apis::urlencode(p_path_instance_uuid));
...
pub async fn managed_blueprints_import_create(configuration: &configuration::Configuration, file: Option<std::path::PathBuf>, path: Option<&str>) -> Result<models::BlueprintImportResult, Error<ManagedBlueprintsImportCreateError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_form_file = file;
    let p_form_path = path;

    let uri_str = format!("{}/managed/blueprints/import/", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::POST, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ManagedBlueprintsImportCreateError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn managed_blueprints_used_by_list(configuration: &configuration::Configuration, instance_uuid: &str) -> Result<Vec<models::UsedBy>, Error<ManagedBlueprintsUsedByListError>> {
    ...
    let uri_str = format!("{}/managed/blueprints/{instance_uuid}/used_by/", configuration.base_path, instance_uuid=crate::apis::urlencode(p_path_instance_uuid));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
