...
/// Trigger account lockdown for a user.  If no user is specified, locks the current user
/// (self-service). When targeting another user, admin permissions are required.  Returns a flow URL
/// for the frontend to redirect to.
pub async fn core_users_account_lockdown_create(
    configuration: &configuration::Configuration,
    user_account_lockdown_request: Option<models::UserAccountLockdownRequest>,
) -> Result<models::AccountLockdownFlowResponse, Error<CoreUsersAccountLockdownCreateError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => {
                return Err(Error::from(serde_json::Error::custom(
                    "Received `text/plain` content type response that cannot be converted to \
                     `models::AccountLockdownFlowResponse`",
                )));
            }
            ContentType::Unsupported(unknown_type) => {
                return Err(Error::from(serde_json::Error::custom(format!(
        ...
        let content = resp.text().await?;
        let entity: Option<CoreUsersAccountLockdownCreateError> =
            serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
...
>>>>>>> 27fd8cd620 (fix podman wrapper on my side and regen clients)
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id = id;
